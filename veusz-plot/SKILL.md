...
Set('Border/hide', True); Set('horzPosn', 'right'); Set('vertPosn', 'top')   # 位置: left/right/centre x top/bottom/centre
To('/')
...
- **bar**（柱状）：`lengths`(默认 ('y',)，多数据集元组)；`mode`(grouped/stacked/stacked-area)、`direction`、`barfill`、`errorstyle`(none/bar/barends)；`BarFill/fills`、`BarLine/lines`。
...
2. 图例位置：`horzPosn`/`vertPosn`（不是 posn）。
